(function(wHandle, wjQuery) {
    ...
    function drawChatBoard() {
        ...
        for (var i = 0; i < (len - from); i++) {
			...
			//console.log(chatName.setValue(chatBoard[i + from].name));
    ...
    function updateNodes(view, offset) {
        ...
        for (var i = 0;;) {
            ...
            // for (var chars, sym = "";;) { // nick name
            //     chars = view.getUint16(offset, true);
            //     offset += 2;
            //     console.log('chars',chars)
            //     console.log('chars',String.fromCharCode(chars))
            //     if (0 == chars) break;
            //     sym += String.fromCharCode(chars);
            // }
            var node = null;
            if (nodes.hasOwnProperty(nodeid)) {
                ...
                node = new Cell(nodeid, posX, posY, size, colorstr, name, _skin,0,_sym);
                nodelist.push(node);
                nodes[nodeid] = node;
                node.ka = posX;
                node.la = posY;
    ...
    function Cell(uid, ux, uy, usize, ucolor, uname, a,orbitAngle,sym) {
        this.id = uid;
        this.ox = this.x = ux;
        this.oy = this.y = uy;
    ...
    wHandle.setNick = function(arg) {
        ...
        let sym = JSON.parse(this.localStorage.getItem('loginCache')).sym
        hideOverlays();
        
        userNickName = `${sym}_{${skin_name}}${arg}`;
        sendNickName();
        userScore = 0
    };
            ...
            if (this.shouldRender()) {
                ...
                if (0 != this.id) {
                    ...
                    if ((showName || c) && this.name && this.nameCache && (null == e || -1 == knownNameDict_noDisp.indexOf(skinName))) {
						...
						ncache.testt(ctx,~~this.x, ~~this.y, this.orbitAngle, this.size,this.sym);
                    }
					//console.log(UText.setStrokeColor)
    ...
    UText.prototype = {
		...
		testt:function(ctx, xo, yo, orbitAngle, size, symVar) {
                            ...
                            for (var p = 0; p < arrSym.length; p++) {
                                var sym = arrSym[p];
                                var x = xo + Math.cos(orbitAngle + (p * _2PI / arrSym.length)) * (size * 0.5);
                                var y = yo + Math.sin(orbitAngle + (p * _2PI / arrSym.length)) * (size * 0.5);
                                ...
                                ctx.fillStyle = "blue";
                                ctx.beginPath();
                                ctx.font = (size * 0.1) + "px Ubuntu";
                                ctx.fillText(symUnicode[sym], x, y);
                                ctx.fill();
                                ctx.stroke();
                                ctx.closePath();
                            }
...
function keydown(event) {
    ...
    if (event.keyCode == 32) { //key A
        //split();
        setTimeout($("body").trigger($.Event("keyup", { keyCode: 32})), speed*10);
    }
